...
const MenuPage: React.FC<MenuPageProps> = ({ 
  ...
  useEffect(() => {
    ...
    const handleProductsUpdate = (event: CustomEvent) => {
      if (event.detail.products) {
        updateMenuFromProducts(event.detail.products);
      }
    };
  ...
  const loadMenuFromLocalStorage = () => {
    ...
    if (savedProducts) {
      try {
        ...
        updateMenuFromProducts(products);
      } catch (error) {
  ...
  const initializeDefaultMenu = () => {
    const defaultData: { [key: string]: MenuItem[] } = {
      'Coffee': [
  ...
  const updateMenuFromProducts = (products: any[]) => {
    const newMenuData: { [key: string]: MenuItem[] } = {};
    ...
    products.forEach(product => {
      ...
      if (!newMenuData[product.category]) {
        newMenuData[product.category] = [];
      }
      newMenuData[product.category].push(menuItem);
    });
